...
from assertions import assert_not_running, assert_bootstrap_state, assert_all
...
class TestReplaceAddress(Tester):
    ...
    def replace_with_insufficient_replicas_test(self):
        ...
        debug("Inserting Data...")
        node1.stress(['write', 'n=10K', '-schema', 'replication(factor=2)'])
    ...
    def multi_dc_replace_with_rf1_test(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w+') as stress_config:
            ...
            stress_config.flush()
            node1.stress(['user', 'profile=' + stress_config.name, 'n=10k',
                          'ops(insert=1)', '-rate', 'threads=50'])

        session = self.patient_cql_connection(node1)

        # on 2.2+ change system_auth rf to 2 so it doesn't emit unsufficient sources for streaming warning
