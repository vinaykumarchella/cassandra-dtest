...
from assertions import assert_almost_equal, assert_one, assert_bootstrap_state, assert_not_running
...
class TestBootstrap(Tester):
    ...
    def simple_bootstrap_test_nodata(self):
        ...
        assert_bootstrap_state(session, 'COMPLETED')
    ...
    def _bootstrap_test_with_replica_down(self, consistent_range_movement,
        ...
        # on 2.2+ change system_auth rf to 2 so it doesn't show unable
        # to find sufficient sources for streaming warning
    ...
    def resumable_bootstrap_test(self):
        ...
        session = self.exclusive_cql_connection(node3)
        assert_bootstrap_state(session, 'IN_PROGRESS')
    ...
    def local_quorum_bootstrap_test(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w+') as stress_config:
            ...
            stress_config.flush()
            node1.stress(['user', 'profile=' + stress_config.name, 'n=2000000', 'no-warmup',
